<?php
	...
	$cstaffdept = $_SESSION['spldept']; //getting staff dept
	...
	{
		//echo strcmp(strtotime($endTime), strtotime("9:15"));
		...
		else if(strcmp(strtotime($endTime), strtotime("16:00")) > 0 && strcmp($_SESSION['periodofbooking'], 'p8'))
		{
			$pastbook = 2;
		}
	}
          ...
          <div class="card rounded shadow " style="background-color: #fefefe; ">
            ...
            ?>
					...
					<div class="form-group">
				 	<label for="dept">Department booking for</label>
			  		<select class="custom-select mb-2" style="border-radius: 25px;" id="dept" name="dept">
					...
					<?php
						...
						{
					?>
						<label class="mt-3" for="password">Event to be held</label>
					  	<input type="text" class="form-control" id="password" style="border-radius: 25px;" aria-describedby="emailHelp" placeholder="Enter event name" name="topic" required="true">
					<?php
						}
					?>
					</div>
					...
					<form class="col-12 pl-lg-4 pr-lg-4 pt-lg-2" action="dashsep.php">
						<div class="d-flex justify-content-center">
						  <button class="btn btn-outline-danger btn-block mb-2 " onclick="location.href = 'dashsep.php';" style="border-radius: 25px;">Go Back</button>  
						</div>	
					</form>
