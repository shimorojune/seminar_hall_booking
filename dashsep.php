<?php
	...
	if(isset($_POST['dept']))
	{
		$cstaffdept = $_POST['dept']; //getting staff dept
		$_SESSION['spldept'] = $cstaffdept; //setting dept session for booking purposes
	}
	else
	{
		$cstaffdept = $_SESSION['spldept']; //getting value if there is no POST available
	}	
	...
	//$typeofbooking = $_POST['tob']; //getting the type of booking
	if(isset($_POST['tob']))
	{
		$typeofbooking = $_POST['tob']; //getting type of booking
		$_SESSION['spltob'] = $typeofbooking; //setting tob session for booking purposes
	}
	else
	{
		$typeofbooking = $_SESSION['spltob']; //getting value if there is no POST available
	}
	$dte=date("Y-m-d"); //getting current date
	//$dte = date('Y-m-d', strtotime($dte. ' + 3 days'));
	...
	//$fdate = date('Y-m-d', strtotime($dte. ' - 5 days')); //setting starting date for finding 1st Day order
  ...
  <body style="background-color: #5576C4;">
    <nav class="navbar navbar-expand-lg navbar-light bg-light" style="background-color: #fefefe;">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        ?>
          <li class="nav-item active">
            <a class="nav-link" href="sepbook.php">Back</a>
          </li>
        ...
        <tbody>
          ...
          <tr class="table-light">
            ...
            <?php
                ...
                { 
            ?>
                    <td style="word-wrap: break-word;"><form method="POST" action="bdetailsep.php"><input type="hidden" name="dates" value="<?php echo $row['DOE'];?>" /><input  type="hidden" name="dept" value="<?php echo $cstaffdept;?>" /><input type="hidden" name="dayorder" value="<?php echo $dayorder;?>" /><input  type="hidden" name="one" value="<?php echo $ppos[$l];?>" /><input 
            <?php
                      if($disableflag == 1)
                      {
            ?>
